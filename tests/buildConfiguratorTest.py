...
class BuildConfiguratorTest(TestCase):
    ...
    def test_build_configuration_generated_with_custom_commands(self) -> None:
        ...
        self.assertTrue(compare_files(f'{TEST_RESOURCE_ROOT}/expected/pre-install.sh',
                                      f'{generated_stage_path}/01-run-chroot.sh'))
        self.assertTrue(compare_files(f'{TEST_RESOURCE_ROOT}/expected/post-install.sh',
                                      f'{generated_stage_path}/03-run-chroot.sh'))


def create_target_config() -> TargetConfig:
    return TargetConfig(name='test-target', version='1.0.0', reference='test-ref',
